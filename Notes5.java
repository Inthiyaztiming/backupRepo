...
//-----------------Managing password-----------------------------
//We manage passwords differently based on the environment.
//For local test runs, we use a config.properties file placed under src/test/resources, where we store app.url, browser, credentials
//To avoid accidentally pushing sensitive data, we add this file to .gitignore, so Git doesn’t track it.
//In CI/CD or production-like environments- we use Jenkins Credentials Plugin(Refer Step by step explanation in chatgpt)
...
// main/java — In this, we write logic for each web page.
// It contains classes with element locators and related action methods using @FindBy annotations.
// This follows the Page Object Model (POM) design pattern.

// test/java — In this package, we write the actual test cases or test logic.
// These test classes call methods from the page classes written in main/java.
...
//While I have primarily worked with manual and scheduled triggers, I understand the concept of automatic triggering 
//using webhooks on Git push events.
//I am eager to set up webhook-based automation when the opportunity arises.
